...
*Archived from `TODO-CLAUDE-CODE.md` on 2026-08-23, at E's direction.*

**Nothing in this file is a work item.** It holds every completed block, plus every block written
against a backend the app no longer has — Supabase, Cognito/AWS and the Poke/DynamoDB reminders
bridge, all deleted in `5244650` when the app cut over to Firebase.

It is kept because the blocks record WHY decisions were made, and several carry device-verification
trails that would be expensive to reconstruct. Read it as history. If something here looks like an
outstanding task, it is not: check the live file, or ask E.

Blocks appear in their original order.
