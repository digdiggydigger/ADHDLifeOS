...
final class ConfirmCelebrationTimingTests: XCTestCase {
    ...
    private func burst(
        _ ordinal: Int, after seconds: TimeInterval = 0, clearedStack: Bool = false
    ) -> CelebrationBurst {
        CelebrationBurst(
            ordinal: ordinal, kind: .confirm(clearedStack: clearedStack), surface: .root,
            start: launch.addingTimeInterval(seconds), origin: nil
        )
    }
    ...
    func testTheCelebrationRunsTheExtraSecondsEAskedFor() {
        let confirm = burst(1)
        XCTAssertEqual(
            ConfirmCelebrationClock.extraLength, 1.2, accuracy: 1e-9,
            "The celebration is not the 1.2 s longer E asked for."
        )
        XCTAssertEqual(ConfirmCelebrationClock.everyConfirmLength, 5.4, accuracy: 1e-9)
        XCTAssertEqual(
            CelebrationQueue.choreographyTime(of: confirm, at: launch.addingTimeInterval(5.4)), 4.2, accuracy: 1e-6,
            "The whole prototype choreography does not fit the longer celebration exactly."
        )
        XCTAssertEqual(
            CelebrationQueue.choreographyTime(of: confirm, at: launch.addingTimeInterval(2.7)), 2.1, accuracy: 1e-6,
    ...
    func testAConfirmBurstLastsUntilItsLastPieceHasLanded() {
        ...
        let lastLanding = (pieces.map { $0.delay + $0.lifetime }.max() ?? 0) / ConfirmCelebrationClock.pace
        XCTAssertEqual(ConfirmCelebrationClock.everyConfirmLength, 5.4, accuracy: 1e-9)
        XCTAssertLessThanOrEqual(
            lastLanding, ConfirmCelebrationClock.everyConfirmLength,
    ...
    func testAFourthQuickConfirmDropsTheOldestBurst() {
        var bursts: [CelebrationBurst] = []
        for ordinal in 1...4 {
            let next = burst(ordinal, after: 0.2 * Double(ordinal))
            bursts = CelebrationQueue.adding(next, to: bursts, now: next.start)
        }
        XCTAssertEqual(
    ...
    func testABurstIsGoneOnceItsLengthHasPassed() {
        let first = burst(1)
        XCTAssertEqual(
            CelebrationQueue.pruned([first], now: launch.addingTimeInterval(5.39)).map(\.ordinal), [1]
        )
        XCTAssertEqual(
            CelebrationQueue.pruned([first], now: launch.addingTimeInterval(5.41)).count, 0,
            "A finished burst stays live, so the layer keeps redrawing an empty sky."
        )
        let later = burst(2, after: 6)
        XCTAssertEqual(
            CelebrationQueue.adding(later, to: [first], now: later.start).map(\.ordinal), [2],
            "Adding a burst keeps one that had already ended."
        )
        XCTAssertEqual(
            CelebrationQueue.nextExpiry(of: [first, burst(3, after: 1)])?
                .timeIntervalSince(launch) ?? 0,
            5.4, accuracy: 1e-6,
    ...
    func testAStackClearingConfirmPlaysItsFireworksOnTheSameStretch() {
        let cleared = burst(1, clearedStack: true)
        XCTAssertEqual(ConfirmCelebrationClock.stackClearingChoreographyLength, 5.0, accuracy: 1e-9)
        XCTAssertEqual(
            CelebrationQueue.length(of: cleared), 5.0 / ConfirmCelebrationClock.pace, accuracy: 1e-9,
            "The fireworks keep their own clock. E chose \"Same stretch\"."
        )
        XCTAssertEqual(CelebrationQueue.length(of: cleared), 6.43, accuracy: 0.005)
        XCTAssertEqual(
            CelebrationQueue.length(of: burst(1)), 5.4, accuracy: 1e-9,
            "An every-Confirm is now longer than E saw."
        )
        let end = launch.addingTimeInterval(CelebrationQueue.length(of: cleared))
        XCTAssertEqual(
            CelebrationQueue.choreographyTime(of: cleared, at: end), 5.0, accuracy: 1e-6,
    ...
    func testAStackClearingBurstOutlivesItsLastSparkAndItsDim() {
        XCTAssertLessThanOrEqual(
            ConfirmFireworksSchedule.lastSparkTime, ConfirmCelebrationClock.stackClearingChoreographyLength
        )
        XCTAssertLessThanOrEqual(
            ConfirmCelebrationDim.goneBy, ConfirmCelebrationClock.stackClearingChoreographyLength,
        ...
        let cleared = burst(1, clearedStack: true)
        XCTAssertEqual(
            CelebrationQueue.pruned([cleared], now: launch.addingTimeInterval(6.42)).count, 1,
            "A stack-clearing burst is removed while its dim is still lifting."
        )
        XCTAssertEqual(CelebrationQueue.pruned([cleared], now: launch.addingTimeInterval(6.44)).count, 0)
        XCTAssertEqual(
            CelebrationQueue.nextExpiry(of: [cleared, burst(2)])?.timeIntervalSince(launch) ?? 0, 5.4, accuracy: 1e-6,
