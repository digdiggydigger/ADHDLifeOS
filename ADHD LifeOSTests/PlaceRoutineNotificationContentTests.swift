...
final class PlaceRoutineNotificationContentTests: XCTestCase {
    ...
    func testUserInfo_carriesTheRunUUIDAndNothingElse() {
        ...
        XCTAssertEqual(
            ...
            "the store is the single source of truth — the tap carries only the minted key,"
                + " and a mismatch IS the stale-tap rule"
        )
    }

    // MARK: - The words

    func testNotification_absorbsMessageAndReport_thenOffersTheSteps() {
        ...
        let content = PlaceRoutineNotificationContent.notification(
            for: run, ranLines: ["Journaled \u{201C}Leg day\u{201D}"]
        )
    ...
    func testNotification_withoutMessageOrReport_isJustTheSteps() {
        let run = makeRun(actions: [spotify(), text()])

        let content = PlaceRoutineNotificationContent.notification(for: run, ranLines: [])
